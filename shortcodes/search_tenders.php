<?php
if ($atts['class']) { $form_wrap_class = $atts["class"].'"';}
?>
...
if(!$atts['market'] || count($markets)>1 ) {
 ...
 /*Country list */
  ...
  $countries = explode (',', $str); }
   ...
   } elseif (count($countries)>1) { $chosen_countries = $countries;  }
...
} elseif (count($products)>1) { $chosen_products = $products;
...
<?php }
  ...
  $statuses   = explode (',', $str);
   if(!$atts['tender-status'] || count($statuses)>1 ) {
        ...
        if (!$atts['tender-status'] || in_array('current', $statuses)){ echo '<option value="current">'.__( '- current tenders', 'kadence-child' ).'</option>'; }
        if (!$atts['tender-status'] || in_array('upcoming', $statuses)){   echo '<option value="upcoming">'.__( '- upcoming tenders', 'kadence-child' ).'</option>'; }
        if (!$atts['tender-status'] || in_array('closed', $statuses)){    echo '<option value="closed">'.__( 'Closed tenders', 'kadence-child' ).'</option>'; }
        echo '</select>';
