...
function kadence_child_action_register_sidebars()
{
	$widgets = array(
    	...
    	'tender-banner'      => __('Banner for specific tender', 'kadence-child'),
	);
...
<?php $forms_content = '
	<div id="fancybox_form" style="width:320px;">
		...
		<div class="register_form_fancy_link2 reg_link"><a href="#" class="register-btn">' . esc_html__('New User Registration', 'kadence-child') . '</a></div>
...
<?php $forms_content = '
	<div id="fancybox_form" style="width:320px;">
		...
		<div class="register_form_fancy_link2 log_link"><a href="#" class="login-btn">' . esc_html__('Already registered? Sign in', 'kadence-child') . '</a></div>
		<div class="recover_passwd"><a href="/client-area/member-password-lost/">Recover password</a> </div>
	</div>';
	return $forms_content;
}
...
/**
 * Extend WordPress login session duration to 3 months
 */

// Extend auth cookie expiration
add_filter('auth_cookie_expiration', 'extend_user_session_expiration', 99, 3);
function extend_user_session_expiration($expiration, $user_id, $remember) {
    // Set expiration to 3 months (in seconds)
    // 60 seconds * 60 minutes * 24 hours * 90 days
    return 60 * 60 * 24 * 90;
}

// Prevent session cleanup
add_filter('wp_session_expiration', 'extend_wp_session_expiration');
function extend_wp_session_expiration() {
    return 60 * 60 * 24 * 90; // 90 days in seconds
}

// Modify remember me checkbox to always be checked
add_action('login_head', 'auto_check_remember_me');
...
function custom_login_logo() {
    ...
    <style type="text/css">
        #login h1 a, .login h1 a {
            background-image: url(<?php echo get_stylesheet_directory_uri(); ?>/images/concealedwineslogotype.png);
            height: 65px;
            width: 320px;
		...
		.wp-core-ui .button-primary {
   		 	background: #f3570dcc !important;
   		 	border-color: #f3570dcc !important;
    		color: #fff;
	}
    </style>
    <?php
